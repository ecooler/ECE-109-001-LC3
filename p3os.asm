...
USER_STACK  .FILL xC000 ; bottom of user stack
...
TRAP_PRINTN  ; prints R0 as a decimal number
    ; save registers
    ST R0, OS_SAVE_R0
    ST R1, OS_SAVE_R1
    ST R2, OS_SAVE_R2
    ST R3, OS_SAVE_R3
    ST R5, OS_SAVE_R5
    ST R7, TRAP_PRINTN_SAVE_R7
    
    AND R5, R5, #0   ; R5 remembers if negative
    ADD R0, R0, #0
    BRzp TRAP_PRINTN_POS
    ADD R5, R5, #1
    NOT R0, R0       ; make it positive
    ADD R0, R0, #1

TRAP_PRINTN_POS
    AND R2, R2, #0   ; initialize quotient to zero
    LEA R1, TRAP_PRINTN_BOS
    
TRAP_PRINTN_DIV10
    ADD R3, R0, #-10 ; check for remainder
    BRzp TRAP_PRINTN_DIV
    LD  R3, TRAP_PRINTN_ZERO  ; push digit character
    ADD R0, R0, R3
    ADD R1, R1, #-1
    STR R0, R1, #0
    BRnzp TRAP_PRINTN_NEXT
TRAP_PRINTN_DIV
    ADD R2, R2, #1   ; accumulate quotient
    ADD R0, R3, #0
    BRnzp TRAP_PRINTN_DIV10
TRAP_PRINTN_NEXT
    ADD R2, R2, #0   ; check for zero quotient
    BRz TRAP_PRINTN_PRINT
    ADD R0, R2, #0   ; next digit
    AND R2, R2, #0
    BRnzp TRAP_PRINTN_DIV10
    
TRAP_PRINTN_PRINT
    LD R2, TRAP_PRINTN_ZERO
    ADD R5, R5, #0  ; print minus, if needed
    BRz TRAP_PRINTN_LOOP
    LD R0, TRAP_PRINTN_MINUS
    OUT
TRAP_PRINTN_LOOP
    LDR R0, R1, #0   ; pop value
    BRz TRAP_PRINTN_DONE  ; stop when null char
    ADD R1, R1, #1
    OUT              ; print
    BRnzp TRAP_PRINTN_LOOP
...
TRAP_PRINTN_DONE
    ...
    LD R3, OS_SAVE_R3
    LD R5, OS_SAVE_R5
    LD R7, TRAP_PRINTN_SAVE_R7
    RET

TRAP_PRINTN_STACK   .BLKW 5
TRAP_PRINTN_BOS     .FILL 0
TRAP_PRINTN_SAVE_R7 .BLKW 1
TRAP_PRINTN_ZERO    .FILL x30  ; '0'
TRAP_PRINTN_MINUS   .FILL x2D  ; '-'
